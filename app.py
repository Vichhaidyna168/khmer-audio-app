...
def get_working_gemini_model_name(selected_name):
    clean = selected_name.lower().strip()
    if "pro" in clean:
...
def generate_synced_audio_km(srt_raw_text, voice_mode_selected):
    ...
    try:
        ...
        raise Exception("ទម្រង់ SRT មានបញ្ហា! សូមពិនិត្យមើល Timecode SRT ឡើងវិញ។")
    ...
    for idx, sub in enumerate(subs):
        ...
        # បញ្ចូលចន្លោះស្ងាត់ (Silence Gap) តាម Timecode ដើមនៃ SRT
        ...
        if cleaned_text:
            ...
            success = loop.run_until_complete(generate_edge_audio_file(cleaned_text, chosen_voice, audio_path))
            if not success:
                try:
    ...
    # បញ្ចូល File MP3 សំឡេង និង Silence ទាំងអស់ចូលគ្នាដោយប្រើ FFmpeg concat
