...
def generate_silence_mp3(duration_sec):
    """ បង្កើតសំឡេងស្ងាត់ MP3 ដោយប្រើ FFmpeg (មានស្រាប់ក្នុង Streamlit Cloud) """
    if duration_sec <= 0:
        return b""
    out_file = tempfile.mktemp(suffix=".mp3")
    cmd = [
        "ffmpeg", "-y", "-f", "lavfi", "-i", "anullsrc=r=24000:cl=mono",
        "-t", str(duration_sec), "-q:a", "9", "-acodec", "libmp3lame", out_file
    ]
    try:
...
def get_audio_duration_ffmpeg(file_path):
    """ គណនាប្រវែងសំឡេងពិតប្រាកដជាវិនាទី """
...
def generate_synced_audio_km(srt_raw_text, voice_mode_selected):
    ...
    for idx, sub in enumerate(subs):
        ...
        # ១. បន្ថែមចន្លោះស្ងាត់ (Silence Gap) ប្រសិនបើ Timecode ដើមមានចន្លោះទំនេរ
        if start_sec > current_time_sec:
            silence_dur = start_sec - current_time_sec
            silence_bytes = generate_silence_mp3(silence_dur)
            if silence_bytes:
        ...
        if cleaned_text:
            audio_bytes = fetch_edge_audio_sync(cleaned_text, chosen_voice, rate_percent=rate_percent)
            if audio_bytes:
                audio_chunks.append(audio_bytes)

                # គណនាប្រវែងសំឡេងដែលបានបន្ថែមដើម្បីដឹងម៉ោងបច្ចុប្បន្ន
                with tempfile.NamedTemporaryFile(delete=False, suffix=".mp3") as tmp:
                    tmp.write(audio_bytes)
                    tmp_p = tmp.name
                dur = get_audio_duration_ffmpeg(tmp_p)
                if os.path.exists(tmp_p):
                    os.remove(tmp_p)
                
                current_time_sec += dur if dur > 0 else duration_sec
