...
def generate_clean_mp3_silence(duration_sec):
    """បង្កាត់ Silence Bytes MP3 ស្ដង់ដារ (MPEG-1 Layer 3, 24kHz, 48kbps Mono)"""
...
async def generate_synced_audio(srt_raw_text, voice_mode_selected):
    ...
    for idx, sub in enumerate(subs):
        ...
        # 1. បន្ថែម Silence Gap ប្រសិនបើពេលវេលានៅចន្លោះSub ទំនេរ
        ...
        if cleaned_text and not (
            ...
            if audio_chunk:
                ...
                # បើទាញសំឡេងមិនបាន ជំនួសដោយ Silence ឱ្យស្មើប្រវែង Subtitle
                combined_bytes.extend(
                    generate_clean_mp3_silence(end_sec - start_sec)
                )
                current_time_sec = end_sec
        else:
            # ករណីជាអក្សរក្នុងវង់ក្រចក (អត់មានសំឡេងនិយាយ) ថែម Silence ឱ្យគ្រប់នាទី
            combined_bytes.extend(
                generate_clean_mp3_silence(end_sec - start_sec)
            )
            current_time_sec = end_sec
...
with st.sidebar:
    ...
    # ចំណុចកែសម្រួល៖ រក្សាទុក API Key ក្នុង Session State មិនឱ្យបាត់ពេល Refresh
    if "stored_api_key" not in st.session_state:
        st.session_state["stored_api_key"] = st.secrets.get(
            "GEMINI_API_KEY", ""
        )
    ...
    api_input = st.text_area(
        ...
        key="api_key_textarea",
    )

    if api_input != st.session_state["stored_api_key"]:
        st.session_state["stored_api_key"] = api_input

    active_api_key = None
    if st.session_state["stored_api_key"].strip():
        keys_list = [
            k.strip()
            for k in st.session_state["stored_api_key"].split("\n")
            if k.strip()
        ]
        st.success(f"✅ កំពុងប្រើប្រាស់ {len(keys_list)} Keys (រក្សាទុកជាប់)")
