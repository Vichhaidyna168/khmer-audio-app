...
def fetch_edge_audio_sync(text, voice):
    """ ទាញយកសំឡេង Edge-TTS ដោយផ្ទាល់ មិនឱ្យគាំងនៅលើ Streamlit Cloud """
    temp_file = tempfile.mktemp(suffix=".mp3")
    cmd = ["edge-tts", "--voice", voice, "--text", text, "--write-media", temp_file]
    try:
...
def generate_synced_audio_km(srt_raw_text, voice_mode_selected):
    ...
    # បង្កើត Folder បណ្ដោះអាសន្នសម្រាប់ទុកហ្វាលសំឡេងនីមួយៗ
    ...
    # ២. ប្រើប្រាស់ FFmpeg តភ្ជាប់ហ្វាល (Concat Audio) ឱ្យចេញជា MP3 រលូនស្តង់ដារ មិនទាក់
    output_merged_path = os.path.join(temp_dir, "final_dubbed.mp3")
    cmd_concat = [
        "ffmpeg", "-y", "-f", "concat", "-safe", "0", "-i", concat_list_path,
        "-c:a", "libmp3lame", "-b:a", "128k", output_merged_path
    ]
    subprocess.run(cmd_concat, stdout=subprocess.PIPE, stderr=subprocess.PIPE)

    combined = b""
    if os.path.exists(output_merged_path):
        with open(output_merged_path, "rb") as f_final:
            combined = f_final.read()
