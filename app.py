...
def fetch_tts_audio_safe(text, voice, out_path):
    """ ទាញយកសំឡេងដោយសុវត្ថិភាព តាម Edge-TTS CLI ឬ gTTS Fallback """
    # វិធីទី ១៖ ប្រើ edge-tts via Subprocess CLI
    cmd = ["edge-tts", "--voice", voice, "--text", text, "--write-media", out_path]
    try:
        res = subprocess.run(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, timeout=15)
        if res.returncode == 0 and os.path.exists(out_path) and os.path.getsize(out_path) > 200:
            return True
    except Exception:
        pass

    # វិធីទី ២៖ ប្រើ gTTS Fallback សុវត្ថិភាព ការពារ Crash
    try:
...
def generate_synced_audio_km(srt_raw_text, voice_mode_selected):
    ...
    try:
        ...
        raise Exception("ទម្រង់ SRT មានបញ្ហា! សូមពិនិត្យមើល Timecode នៃ SRT ឡើងវិញ civilization។")
    ...
    for idx, sub in enumerate(subs):
        ...
        if cleaned_text:
            ...
            success = fetch_tts_audio_safe(cleaned_text, chosen_voice, raw_audio_path)
            if success:
