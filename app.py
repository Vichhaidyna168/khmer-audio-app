...
async def get_audio_bytes(text, voice_code, target_lang="Khmer (ខ្មែរ)", retries=3):
    # បើអក្សរជាភាសាខ្មែរ ប្រើ gTTS (Google Khmer TTS) ជានិច្ចដើម្បីឱ្យឮសំឡេងច្បាស់ ១00%
    if "Khmer" in target_lang or re.search(r'[\u1780-\u17FF]', text):
...
async def generate_synced_audio(srt_raw_text, voice_mode_selected, target_lang_selected):
    ...
    for idx, sub in enumerate(valid_subs):
        ...
        if cleaned_text:
            ...
            audio_chunk = await get_audio_bytes(cleaned_text, v_code, target_lang=target_lang_selected)
            if audio_chunk:
        ...
        if not srt_content.strip():
            ...
            try:
                audio_bytes = run_async(
                    generate_synced_audio(srt_content, voice_mode, target_lang)
                )
...
# --- TAB 3: SUBTITLE TO VIDEO GENERATOR ---
with tab3:
        ...
        if not v_input or not s_input:
            ...
            try:
                with st.spinner("កំពុងដំណើរការបង្កប់ Subtitle ចូលក្នុងវីដេអូ..."):
                    ...
                    # ប្រើប្រាស់ FFmpeg ដើម្បីបង្កប់ Subtitle ចូលក្នុង Video
                    cmd = [
                        "ffmpeg", "-y", "-i", v_path,
                    ...
                    if os.path.exists(out_video_path) and os.path.getsize(out_video_path) > 1000:
                        ...
                        st.error("❌ កើតមានបញ្ហាពេលបង្កប់ Subtitle! សូមពិនិត្យមើលទម្រង់ SRT File ឡើងវិញ។")
