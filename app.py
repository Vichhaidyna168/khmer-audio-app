...
def polish_srt_with_gemini(srt_text, api_key, model_name):
    ...
    ៥. ហាមបន្ថែមអក្សរនាំមុខ បង្ហាញតែទម្រង់ SRT ដែលបានកែរួចប៉ុណ្ណោះ។

    អត្ថបទ SRT ដើម៖
    {srt_text}
    """

    # ១. សាកល្បងហៅ Model ដែលជ្រើសរើសជាមុន
    try:
    ...
    # ២. បើ Model ខាងលើលោត Error 404 ឱ្យប្រព័ន្ធស្វែងរក Model ណាដែលដំណើរការស្វ័យប្រវត្តិ
    try:
        available_models = [
            m.name
        ...
        raise Exception(f"មិនអាចភ្ជាប់ទៅ Google Gemini API បានទេ៖ {e}")

    raise Exception(
        "មិនមាន Model ណាអាចប្រើប្រាស់បានឡើយ! សូមពិនិត្យ API Key របស់អ្នកសារជាថ្មី Security/Quota"
    )
