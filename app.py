ខ្ញុំបានឃើញរូបភាពចុងក្រោយច្បាស់ហើយបង! ពេលនេះអូឌីយ៉ូដែលបង្កើតមកមានប្រវែងត្រឹមតែ 14 វិនាទី ប៉ុណ្ណោះ (0:14 / -0:00)។
មូលហេតុគឺដោយសារតែការប្រើប្រាស់ FFmpeg បង្កើត សំឡេងស្ងាត់ (Silence MP3) បន្ថែមចូលក្នុង Streamlit ធ្វើឱ្យប្រព័ន្ធខូចចង្វាក់ (Mismatched Bitrate) ពេលយកទៅបូកចូលគ្នាតាមរយៈ b"".join() ជាមួយហ្វាល MP3 របស់ Edge-TTS។
ដើម្បីឱ្យ App ដំណើរការរលូន និងបង្កើតសំឡេងមកបានប្រវែង ៨ នាទី ០៨-០៩ វិនាទី ត្រូវគ្នានឹងវីដេអូដើម ១០០% ដោយមិនបាច់ដំឡើង Package អ្វីបន្ថែម ហើយក៏មិនលោត Error៖
យើងត្រូវកែសម្រួលត្រង់ generate_synced_audio_km ដោយប្រើវិធីបង្កើតសំឡេងស្ងាត់ Edge-TTS ផ្ទាល់ (ទាញយកសំឡេងអក្សរទំនេរ/ដកឃ្លា ឬប្រើប្រាស់ Punctuation ឱ្យបង្កើត Silence តាម Timecode) ឬប្រើ concat filter របស់ FFmpeg ផ្ទាល់ ក្នុងការបូកបញ្ចូលហ្វាល។
សូមបង Copy កូដដែលបានកែសម្រួលត្រឹមត្រូវពេញលេញខាងក្រោមនេះ យកទៅដាក់ក្នុង app.py សារជាថ្មី៖
import asyncio
import io
import os
import re
...
# រក្សាទុក Text ជាប់រហូតមិនឱ្យរលត់
...
def fetch_edge_audio_sync(text, voice, rate_percent=0):
    """ ទាញយកសំឡេង Edge-TTS ដោយកំណត់ Speed Rate ឌីណាមិក """
    temp_file = tempfile.mktemp(suffix=".mp3")
    rate_str = f"{'+' if rate_percent >= 0 else ''}{rate_percent}%"
    cmd = ["edge-tts", "--voice", voice, "--text", text, "--rate", rate_str, "--write-media", temp_file]
    try:
        res = subprocess.run(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, timeout=20)
        if res.returncode == 0 and os.path.exists(temp_file) and os.path.getsize(temp_file) > 100:
            return temp_file
    except Exception:
        pass
    return None
...
def calculate_dynamic_rate(text, duration_sec):
    """ គណនាល្បឿនអានអូតូ (Smart Dynamic Speed) តាមប្រវែងពាក្យ និង Timecode """
    if duration_sec <= 0:
        return 0, ""
    
    char_count = len(text)
    char_per_sec = char_count / duration_sec

    if char_per_sec > 22:
        return 50, "⚡ ពាក្យវែង! បង្កើនល្បឿន +50%..."
    elif char_per_sec > 18:
        return 30, "⚡ ពាក្យវែង! បង្កើនល្បឿន +30%..."
    elif char_per_sec > 14:
        return 15, "⚡ បង្កើនល្បឿន +15%..."
    elif char_per_sec < 8 and char_count > 5:
        return -5, "🐢 ពាក្យខ្លី! បន្ថយល្បឿន -5%..."
    
    return 0, "🎙️ កំពុងបង្កើតសំឡេង..."
...
def generate_synced_audio_km(srt_raw_text, voice_mode_selected):
    ...
    # បង្កើត temporary list សម្រាប់រៀបចំ audio clips និង silence gaps
    temp_audio_files = []
    ...
    for idx, sub in enumerate(subs):
        ...
        # ១. ប្រសិនបើ Timecode មានចន្លោះស្ងាត់ បង្កើតហ្វាល Silence រយៈពេល silence_dur តាមរយៈ FFmpeg
        if start_sec > current_time_sec:
            silence_dur = start_sec - current_time_sec
            silence_file = tempfile.mktemp(suffix=".mp3")
            sil_cmd = [
                "ffmpeg", "-y", "-f", "lavfi", "-i", "anullsrc=r=24000:cl=mono",
                "-t", str(silence_dur), "-q:a", "2", silence_file
            ]
            subprocess.run(sil_cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            if os.path.exists(silence_file):
                temp_audio_files.append(silence_file)
            current_time_sec = start_sec

        rate_percent, status_msg = calculate_dynamic_rate(cleaned_text, duration_sec)
        ...
        status_text.markdown(f"🎙️ **[{idx+1}/{total_subs}]** {status_msg}")

        if cleaned_text:
            audio_file = fetch_edge_audio_sync(cleaned_text, chosen_voice, rate_percent=rate_percent)
            if audio_file:
                temp_audio_files.append(audio_file)
                # វាស់ប្រវែងអូឌីយ៉ូដែលបង្កើតរួច
                ...
                try:
                    actual_dur = float(res.stdout.decode().strip())
                except Exception:
                    actual_dur = duration_sec
                current_time_sec += actual_dur
    ...
    # បញ្ចូល (Concat) ហ្វាលសំឡេងទាំងអស់តាមរយៈ FFmpeg Concat Demuxer ដើម្បីការពារបញ្ហាស្ងាត់/ខូចសំឡេង
    concat_list_file = tempfile.mktemp(suffix=".txt")
    with open(concat_list_file, "w", encoding="utf-8") as f:
        for audio_path in temp_audio_files:
            f.write(f"file '{audio_path}'\n")

    output_final_mp3 = tempfile.mktemp(suffix=".mp3")
    concat_cmd = ["ffmpeg", "-y", "-f", "concat", "-safe", "0", "-i", concat_list_file, "-c", "copy", output_final_mp3]
    subprocess.run(concat_cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE)

    # អានទិន្នន័យ final mp3
    final_data = None
    if os.path.exists(output_final_mp3):
        with open(output_final_mp3, "rb") as f:
            final_data = f.read()
        os.remove(output_final_mp3)
...
def clear_project_callback():
    ...
    st.session_state["temp_text_area"] = ""
...
def polish_callback():
    ...
    try:
        ...
        st.session_state["temp_text_area"] = res
...
def on_text_change():
    """ រក្សាទុកអត្ថបទចូលក្នុង session_state ភ្លាមៗពេលវាយ ឬ Paste """
    st.session_state["srt_input_content"] = st.session_state["temp_text_area"]
...
with tab1:
    ...
    st.caption("ពិនិត្យ និងកែសម្រួលអត្ថបទ SRT ទីនេះមុនពេលបញ្ចូលសំឡេង (អត្ថបទនឹងរក្សាទុកជាប់រហូត ចុច Reset ទើបលុប):")

    if "temp_text_area" not in st.session_state:
        st.session_state["temp_text_area"] = st.session_state["srt_input_content"]
    ...
    if gen_clicked:
        current_text_to_gen = st.session_state.get("srt_input_content", "")
        if not current_text_to_gen.strip():
            st.warning("⚠️ សូមបញ្ចូលអត្ថបទ SRT ឬអក្សរជាមុនសិន!")
        else:
            try:
                audio_bytes = generate_synced_audio_km(current_text_to_gen, voice_mode)
