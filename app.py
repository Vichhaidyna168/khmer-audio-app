...
def generate_synced_audio_km(srt_raw_text, voice_mode_selected):
    ...
    # --- ចំណុចកែសម្រួល៖ បង្កើត Audio Timeline ដោយគិតចន្លោះស្ងាត់ (Silence Padding) ---
    combined_timeline = AudioSegment.empty()
    current_time_ms = 0
    ...
    for idx, sub in enumerate(subs):
        ...
        # ១. បន្ថែមចន្លោះស្ងាត់ (Silence) បើ Timecode ចាប់ផ្តើម ធំជាង ម៉ោងបច្ចុប្បន្ន
        if start_ms > current_time_ms:
            silence_gap = start_ms - current_time_ms
            combined_timeline += AudioSegment.silent(duration=silence_gap)
            current_time_ms = start_ms

        # គណនាល្បឿនអានអូតូ (Smart Dynamic Speed)
    ...
    if len(combined_timeline) == 0:
        return None

    # Export ចេញជា Audio Bytes ត្រឡប់ទៅ Streamlit
    buffer = io.BytesIO()
    combined_timeline.export(buffer, format="mp3")
...
with tab1:
    ...
    # បង្កើត temp_text_area សម្រាប់ Sync ជាមួយ srt_input_content
