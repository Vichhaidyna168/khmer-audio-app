...
<style>
    ...
    div.stButton > button[key="polish_script_btn"] {
...
def extract_srt_only(text):
    """កាត់យកតែផ្នែក SRT សុទ្ធ ដោយលុបពាក្យនាំមុខ ឬការបកស្រាយផ្សេងៗចោល"""
    if not text:
        return ""
    # រកកន្លែងចាប់ផ្តើមដំបូងនៃ SRT (ឧទាហរណ៍ "1\n00:00:00,000")
    match = re.search(r'(?:^|\n)(\d+\s*[\r\n]+\d\d:\d\d:\d\d)', text)
    if match:
    ...
    try:
        for m in genai.list_models():
            if "generateContent" in m.supported_generation_methods:
                clean_m_name = m.name.replace("models/", "")
                working_models.append(clean_m_name)
    except Exception:
        working_models = ["gemini-2.0-flash", "gemini-2.5-flash", "gemini-1.5-flash", "gemini-1.5-pro"]
...
def polish_srt_with_gemini(srt_text, api_key, model_name):
    ...
    # ទាញយកឈ្មោះ Active Model ដែលដំណើរការបានពិតប្រាកដក្នុង API Key
    working_models = []
    try:
        for m in genai.list_models():
            if "generateContent" in m.supported_generation_methods:
