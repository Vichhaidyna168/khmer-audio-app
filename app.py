...
# ១. បង្កើត Session State សម្រាប់រក្សាទុក SRT Text មិនឲ្យ Auto Clear
...
def generate_edge_silence_sync(duration_sec, voice):
    """ បង្កើតចន្លោះស្ងាត់ (Silence/Pause) តាមរយៈ Edge-TTS """
    if duration_sec <= 0.1:
        return b""
...
def get_working_gemini_model_name(selected_name):
    """
    ទាញយកឈ្មោះ Model ពី Sidebar មកប្រើដោយផ្ទាល់ មិនប្រើឈ្មោះចាស់ (ដោះស្រាយ Error 404)
    """
    clean = str(selected_name).strip()
    return clean if clean else "gemini-3.8-flash"
...
def polish_srt_with_gemini(srt_text, api_key, model_name):
    ...
    try:
        ...
        try:
            # Fallback ទៅកាន់ gemini-3.8-flash ជម្រើសបម្រុង
...
with tab1:
    ...
    # ២. កែប្រែ Text Area ភ្ជាប់key="srt_input_content" ដោយตรง ដើម្បីការពារ Auto Clear 100%
    st.text_area(
        "", 
        height=220, 
