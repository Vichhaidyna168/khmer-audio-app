...
def polish_srt_with_gemini(srt_text, api_key, model_name):
    ...
    if not clean_key.startswith("AIza"):
        raise Exception(
            "API Key មិនត្រឹមត្រូវទេ! API Key របស់ Gemini ត្រូវតែចាប់ផ្តើមដោយអក្សរ 'AIzaSy...'"
        )
    ...
    for m in models_to_try:
        ...
        try:
            ...
            if "401" in str(e) or "API_KEY_INVALID" in str(e):
                raise Exception(
                    "API Key មិនត្រឹមត្រូវ ឬផុតកំណត់ឡើយ (Error 401)! សូមពិនិត្យ Key ក្នុង Google AI Studio សារជាថ្មី។"
                )
            continue

    raise Exception(f"មិនអាចភ្ជាប់ទៅ Gemini បានទេ៖ {last_err}")
...
with tab1:
    ...
    if "srt_main_val" not in st.session_state:
        st.session_state["srt_main_val"] = default_srt

    # ១. បង្កើតប្រអប់ Text Area ដោយប្រើ value ផ្ទាល់
    srt_content = st.text_area(
        "", value=st.session_state["srt_main_val"], height=180, key="srt_text_box_input"
    )

    # ២. ប៊ូតុង AI Polish Script ដំណើរការរលូន ១០០% ដោយគ្មាន Error
    ...
    with col_polish1:
        if st.button(
            ...
            if not active_api_key:
                st.error(
                    "❌ សូម Paste Gemini API Key (ផ្តើមដោយ AIzaSy...) នៅក្នុង Sidebar ជាមុនសិន!"
                )
                ...
                with st.spinner(
                    ...
                    try:
                        ...
                        st.session_state["srt_main_val"] = polished_res
                        st.success(
