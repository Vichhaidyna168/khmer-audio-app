...
# ទុកទទេ មិនឱ្យមានអក្សរគំរូនាំតែពិបាកលុបដៃ
default_srt = ""


def extract_tag_and_text(raw_text):
    """ បំបែក Tag [M] និង [F] ដើម្បីជ្រើសរើសសំឡេងប្រុស ឬស្រី """
    gender = "MALE"
...
async def generate_edge_audio_bytes(text, voice):
    """ ទាញយកសំឡេងខ្មែរ HD (Piseth / Sreymom) """
    try:
...
def generate_synced_audio_km(srt_raw_text, voice_mode_selected):
    ...
    KHMER_MALE_VOICE = "km-KH-PisethNeural"   # សំឡេងប្រុស
    KHMER_FEMALE_VOICE = "km-KH-SreymomNeural" # សំឡេងស្រី
    ...
    for idx, sub in enumerate(subs):
        ...
        # កំណត់សំឡេងតាម Tag [M] ឬ [F] ឬ Setting ក្នុង Sidebar
        ...
        if cleaned_text:
            audio_chunk = asyncio.run(generate_edge_audio_bytes(cleaned_text, chosen_voice))
            if audio_chunk:
