...
# បន្ថែម pydub និង srt សម្រាប់គណនា Timecode ឱ្យត្រឹមត្រូវ
...
# មុខងារបំប្លែង SRT ទៅជា Audio ដោយរៀបចំតាម Timecode
async def generate_synced_audio(srt_raw_text, voice_mode_selected):
    subs = list(srt.parse(srt_raw_text))
    if not subs:
        return None

    # បង្កើត Base Audio ស្ងាត់ដែលមានប្រវែងស្មើនឹង SRT ចុងក្រោយ + 2 វិនាទី
    total_duration_ms = td_to_ms(subs[-1].end) + 2000
    ...
    for idx, sub in enumerate(subs):
        ...
        if cleaned_text:
            # ជ្រើសរើសប្រភេទសំឡេង (ប្រុស/ស្រី)
            gender = "Male"
            ...
            start_ms = td_to_ms(sub.start)

            # បង្កើតសំឡេងតាមរយៈ Edge-TTS
            communicate = Communicate(cleaned_text, v_code)
            with tempfile.NamedTemporaryFile(delete=False, suffix=".mp3") as fp:
                temp_file = fp.name

            try:
                await communicate.save(temp_file)
                speech_segment = AudioSegment.from_file(temp_file)

                # តម្រឹមសំឡេង AI ចូលចំទីតាំង Timecode (Start Time) នៃ SRT
                base_audio = base_audio.overlay(
                    speech_segment, position=start_ms
                )
    ...
    base_audio.export(output_buffer, format="mp3")
...
with tab1:
    st.markdown(
        "21003<span class='num-badge'>1</span>Generate Subtitles (Khmer"
        " (ខ្មែរ))</h2>",
        unsafe_allow_html=True,
    )
    ...
    if gen_clicked:
        if not srt_content.strip():
            ...
            try:
                # ហៅដំណើរការបង្កើតសំឡេងដែល Sync តាម Timecode
                ...
                if audio_bytes:
                    ...
                    st.error("❌ បរាជ័យ៖ ទម្រង់ SRT មិនត្រឹមត្រូវ!")
            except Exception as e:
                status_box.empty()
