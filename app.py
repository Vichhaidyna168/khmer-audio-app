...
def create_valid_mp3_silence(duration_sec):
    """ បង្កើត MP3 Silence Frame ត្រឹមត្រូវតាមស្តង់ដារ មិនឱ្យ CapCut ថ្លង់ ឬខូចនាទី """
    if duration_sec <= 0.05:
        return b""
    # MP3 Frame header ស្ងាត់ 24kHz Mono 128kbps
    FRAME = b'\xff\xfb\x30\xc4\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00' + (b'\x00' * 397)
    frame_count = int(duration_sec / 0.026)
    return FRAME * max(1, frame_count)
...
def calculate_dynamic_rate(text, duration_sec):
    ...
    # បើអក្សរវែងខ្លាំងធៀបនឹង Timecode បង្កើនល្បឿនអូតូ
    if char_per_sec > 22:
        return 70, "⚡ ពាក្យវែង! បង្កើនល្បឿន +70%..."
    elif char_per_sec > 18:
        return 40, "⚡ ពាក្យវែង! បង្កើនល្បឿន +40%..."
    elif char_per_sec > 14:
        return 20, "⚡ បង្កើនល្បឿន +20%..."
    elif char_per_sec < 8 and char_count > 5:
        return -10, "🐢 ពាក្យខ្លី! បន្ថយល្បឿន -10%..."
...
def generate_synced_audio_km(srt_raw_text, voice_mode_selected):
    ...
    for idx, sub in enumerate(subs):
        ...
        # ១. បង្កើត Silence Gap ត្រឹមត្រូវតាម Timecode
        gap = start_sec - last_end_time
        if gap > 0.05:
            silence_bytes = create_valid_mp3_silence(gap)
            if silence_bytes:
                audio_chunks.append(silence_bytes)

        # ២. គណនាល្បឿនអានអូតូ (Smart Dynamic Speed)
