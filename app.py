...
def generate_edge_silence_sync(duration_sec, voice):
    """ បង្កើតចន្លោះស្ងាត់ (Silence/Pause) តាមរយៈ Edge-TTS ដោយមិនបាច់ប្រើ FFmpeg """
    if duration_sec <= 0.1:
        return b""
    # បង្កើត Pause ដោយប្រើចន្លោះ Space និង Punctuation
    pause_text = "..." + " ... " * int(duration_sec * 2)
    return fetch_edge_audio_sync(pause_text, voice, rate_percent=-50)
...
def generate_synced_audio_km(srt_raw_text, voice_mode_selected):
    ...
    for idx, sub in enumerate(subs):
        ...
        # ១. បន្ថែមចន្លោះស្ងាត់ (Silence Gap) ដោយប្រើ Edge-TTS
        if start_sec > current_time_sec:
            silence_dur = start_sec - current_time_sec
            silence_bytes = generate_edge_silence_sync(silence_dur, chosen_voice)
            if silence_bytes:
