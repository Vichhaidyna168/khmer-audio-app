...
def generate_synced_audio_km(srt_raw_text, voice_mode_selected):
    ...
    for idx, sub in enumerate(subs):
        ...
        # បង្កើតចន្លោះស្ងាត់ (Silence Gap)
    ...
    # បញ្ចូលសំឡេងទាំងអស់ចូលគ្នាដោយប្រើ FFmpeg
    concat_list_file = tempfile.mktemp(suffix=".txt")
    with open(concat_list_file, "w", encoding="utf-8") as f:
        for p in audio_files:
            f.write(f"file '{p}'\n")

    output_mp3 = tempfile.mktemp(suffix=".mp3")
    concat_cmd = ["ffmpeg", "-y", "-f", "concat", "-safe", "0", "-i", concat_list_file, "-acodec", "libmp3lame", output_mp3]
