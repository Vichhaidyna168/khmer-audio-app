...
def fetch_edge_audio_sync(text, voice):
    """ ទាញយកសំឡេង Edge-TTS ដោយផ្ទាល់ """
    temp_file = tempfile.mktemp(suffix=".mp3")
    cmd = ["edge-tts", "--voice", voice, "--text", text, "--rate=-4%", "--write-media", temp_file]
    try:
...
def create_silent_mp3_bytes(duration_sec):
    """ បង្កើត MP3 Silent Frame ស្តង់ដារ (ដើម្បីកុំឱ្យ CapCut ថ្លង់ ឬច្រឡំនាទី) """
    if duration_sec <= 0:
        return b""
    # ប្រើ MP3 Silence Frame ត្រឹមត្រូវ (24kHz Mono MP3 Frame)
    SINGLE_FRAME = b'\xff\xfb\x30\xc4\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00' + (b'\x00' * 397)
    # ១ Frame ស្មើនឹង ~0.024 វិនាទី
    frames_needed = int(duration_sec / 0.024)
    return SINGLE_FRAME * max(1, frames_needed)
...
def generate_synced_audio_km(srt_raw_text, voice_mode_selected):
    ...
    for idx, sub in enumerate(subs):
        ...
        # គណនាចន្លោះស្ងាត់តាម SRT
        start_sec = sub.start.total_seconds()
        gap = start_sec - last_end_time

        if gap > 0.1:
            silence_data = create_silent_mp3_bytes(gap)
            if silence_data:
