...
def make_ffmpeg_silence(duration_sec):
    """ បង្កើត Silence MP3 ដោយប្រើ FFmpeg ដោយផ្ទាល់ ធានាថាឮច្បាស់គ្មានស្ងាត់គ្មានទាស់ """
...
def get_khmer_audio_bytes(text):
    """ បង្កើតសំឡេងនិយាយភាសាខ្មែរច្បាស់ៗដោយប្រើ gTTS """
    try:
...
def generate_synced_audio_km(srt_raw_text):
    ...
    for idx, sub in enumerate(subs):
        ...
        # -------------------------------------------------------------
        # 🔑 បង្កើត Silence Delay តាម Timecode SRT ដើម ចេះដកដង្ហើមតាមចង្វាក់
        # -------------------------------------------------------------
        ...
        if cleaned_text:
            audio_chunk = get_khmer_audio_bytes(cleaned_text)
            if audio_chunk:
...
with tab1:
    ...
    # -------------------------------------------------------------
    # 🔑 រក្សាទុកអត្ថបទ SRT មិនឱ្យ Auto Clear/Reset ចោលឡើយ
    # -------------------------------------------------------------
