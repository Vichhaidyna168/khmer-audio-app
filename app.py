...
def make_mp3_silence_bytes(duration_sec):
    if duration_sec <= 0:
        return b""
    # ២៤ kHz, ២៤ kbps MP3 frame header estimation
    num_frames = int(duration_sec * 38.28)
    return b"\xff\xfb\x90\xc4" + (b"\x00" * 413) * num_frames
...
async def generate_synced_audio(srt_raw_text, voice_mode_selected):
    ...
    for idx, sub in enumerate(subs):
        ...
        # គណនាបន្ថែម Silence ឱ្យត្រូវតាម Timestamp ដើម
        if start_sec > current_time_sec:
            gap = start_sec - current_time_sec
            combined_bytes.extend(make_mp3_silence_bytes(gap))
        ...
        if cleaned_text:
            ...
            try:
                ...
                if audio_chunk:
                    ...
                    # តម្រឹមកាលវិភាគពេលវេលាឱ្យស្មើ End Time នៃ Subtitle
...
with st.sidebar:
    ...
    # ទាញយក API Key ពី Secrets ឬ Environment Variable ជាមុន
...
with tab1:
    ...
    # --- ចំណុចទី ១៖ បង្ហាញ Video Player បន្ទាប់ពី Upload ---
    ...
    if st.button("🌐 បកប្រែដោយ Gemini"):
        if not active_api_key:
            ...
            try:
                ...
                # ប្រើប្រាស់ Model ដែលជ្រើសរើសចេញពី Sidebar
