...
def polish_srt_with_gemini(srt_text, api_key, model_name):
    ...
    គោលការណ៍ម៉ឺងម៉ាត់បំផុត (STRICT RULES):
    ១. រក្សាទម្រង់ SRT (លេខរៀង និង Timestamp ឧទាហរណ៍ 00:00:00,000 --> 00:00:02,000) ឱ្យនៅដដែល ១០០% ហាមលុប ឬកាត់បន្ថយជួរពេលវេលាដាច់ខាត!
    ២. សម្រួលពាក្យពេចន៍ជាភាសាខ្មែរនិយាយធម្មជាតិ (Spoken Khmer) មិនមែនអក្សរស្រាវជ្រាវរឹងកំព្រឹសឡើយ។
    ៣. បន្ថែមសញ្ញា [M] នៅដើមជួរសម្រាប់សំឡេងប្រុស និង [F] សម្រាប់សំឡេងស្រី តាមបរិបទសាច់រឿង (បើមានស្រាប់ ត្រូវកែសម្រួលឱ្យកាន់តែសុក្រឹត)។
    ៤. បន្ថែមសញ្ញាវណ្ណយុត្តិដើម្បីបញ្ជាចង្វាក់ដកដង្ហើម និងអារម្មណ៍ក្នុង TTS៖
       - បន្ថែមសញ្ញាក្បៀស (,) ត្រង់កន្លែងត្រូវបង្អង់សំឡេងដកដង្ហើមខ្លី
       - បន្ថែមសញ្ញាចុចបី (...) ត្រង់កន្លែងក្តុកក្តួល គិត ឬរំភើប
       - បន្ថែមសញ្ញា (!) ឬ (?) ត្រង់កន្លែងខឹង ភ្ញាក់ផ្អើល ឬឆ្ងល់
    ៥. ហាមបន្ថែមពាក្យពន្យល់ ឬអក្សរនាំមុខអ្វីទាំងអស់ បង្ហាញតែទម្រង់ SRT ដែលបានកែរួចប៉ុណ្ណោះ។
    ...
    # ជម្រះកូដ Block ករណី Gemini ចេញជា ```srt
...
async def generate_synced_audio(srt_raw_text, voice_mode_selected):
    ...
    for idx, sub in enumerate(subs):
        ...
        # ១. បន្ថែម Silence Gap ប្រសិនបើពេលវេលាទំនេរ
...
with tab1:
    ...
    # --- បន្ថែមប៊ូតុង AI Polish Script សម្រួលចង្វាក់ និងអារម្មណ៍ ---
            ...
            if not active_api_key:
                ...
                with st.spinner(
                    ...
                    try:
                        ...
                        st.session_state["srt_input_text"] = polished_res
                        st.success(
