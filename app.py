...
def generate_clean_mp3_silence(duration_sec):
    ...
    if duration_sec <= 0.1:
        return b""
    # Valid MP3 Silence Frame (288 Bytes = ~0.024s duration)
...
async def generate_synced_audio(srt_raw_text, voice_mode_selected):
    ...
    for idx, sub in enumerate(subs):
        ...
        # 1. គណនា និងបន្ថែម Silence Gap ប្រសិនបើមានចន្លោះទំនេរ
        if start_sec > current_time_sec:
            gap_sec = start_sec - current_time_sec
            silence_bytes = generate_clean_mp3_silence(gap_sec)
            combined_bytes.extend(silence_bytes)
        ...
        if cleaned_text:
            ...
            if audio_chunk:
                ...
                # រៀបចំកាលវិភាគពេលវេលាឱ្យត្រូវតាម Subtitle End Time
