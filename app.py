...
def make_silence_mp3(duration_sec, out_path):
    """ បង្កើត Silence MP3 សម្រាប់ចង្វាក់ដកដង្ហើម """
...
def generate_synced_audio_km(srt_raw_text):
    ...
    # ៣. ផ្គុំ MP3 ដោយប្រើ FFmpeg (ដកដង្ហើមតាម Timecode)
    ...
    # Fallback Memory Direct ករណីគ្មាន FFmpeg លើ Cloud
    ...
    for sub in subs:
        ...
        if cleaned_text:
            try:
                ...
                fp = io.BytesIO()
                tts.write_to_fp(fp)
                combined_bytes.extend(fp.getvalue())
            except Exception:
                pass
...
def clear_single_callback():
    """ ប៊ូតុង Clear តែមួយគត់សម្រាប់លុបអត្ថបទក្នុង Text Area """
...
with tab1:
    ...
    with col_polish2:
        st.button(
            "🧹 Clear",
            key="tab1_clear_single_btn",
            use_container_width=True,
            on_click=clear_single_callback,
        )
