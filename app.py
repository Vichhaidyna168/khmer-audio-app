...
def fetch_edge_audio_sync(text, voice, rate_str="-5%"):
    """ ទាញយកសំឡេង Edge-TTS ដោយតម្រឹមល្បឿនអានឱ្យបាន ៦ នាទី ២៨ វិនាទី """
    temp_file = tempfile.mktemp(suffix=".mp3")
    # ប្រើប្រាស់ --rate=-5% ដើម្បីបន្ថយល្បឿនអានបន្តិច ធ្វើឱ្យប្រវែងសំឡេងសរុបស្មើ ៦ នាទី ២៨ វិនាទី
...
def generate_synced_audio_km(srt_raw_text, voice_mode_selected, tts_rate="-5%"):
    ...
    for idx, sub in enumerate(subs):
        ...
        if cleaned_text:
            audio_bytes = fetch_edge_audio_sync(cleaned_text, chosen_voice, rate_str=tts_rate)
            if audio_bytes:
...
with tab1:
    ...
    # រក្សាទុកអត្ថបទក្នុង session_state មិនឱ្យបាត់អូតូឡើយ
    ...
    # ជម្រើសសារ៉េល្បឿនសំឡេងតម្រឹមនាទី (Speed Adjustment)
    speed_option = st.selectbox(
        "⏱️ តម្រឹមល្បឿនសំឡេង (TTS Speed Tuning):",
        ["-5% (ស្តង់ដារ ៦ នាទី ២៨ វិនាទី)", "-6% (យឺតបន្តិច)", "-4% (លឿនបន្តិច)", "+0% (ល្បឿនដើម)"],
        index=0,
        key="tab1_speed_tune"
    )
    selected_rate = speed_option.split(" ")[0]
    ...
    if gen_clicked:
        if not srt_content.strip():
            ...
            try:
                audio_bytes = generate_synced_audio_km(srt_content, voice_mode, tts_rate=selected_rate)
