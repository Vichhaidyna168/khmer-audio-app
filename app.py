...
<style>
    ...
    div.stButton > button[key="tab1_clear_text_btn"] {
...
def extract_tag_and_text(raw_text):
    """ បែងចែក Tag [M], [F] ឬ [E] ចេញពីអត្ថបទ """
    gender = "MALE"
...
def make_ffmpeg_silence_file(duration_sec, out_path):
    """ បង្កើត Silence MP3 តាមកម្រិតវិនាទីដែលត្រូវដកដង្ហើម """
...
def get_google_cloud_tts_voice(text, gender, api_key, out_path):
    """ ប្រើប្រាស់ Google Cloud Text-to-Speech API ដើម្បបំបែកសំឡេងប្រុស/ស្រី កម្រិត HD """
    if not api_key:
        return False

    url = f"https://texttospeech.googleapis.com/v1/text:synthesize?key={api_key}"
    
    # កំណត់ Voice Name សម្រាប់សំឡេងខ្មែរ (KM)
    voice_name = "km-KH-Standard-A"  # អាចប្រើប្រាស់ Standard Voice
    ssml_gender = "MALE" if gender == "MALE" else "FEMALE"
    ...
    payload = {
        ...
        "audioConfig": {
            "audioEncoding": "MP3",
            "speakingRate": 0.95, # កំណត់ល្បឿនអានឱ្យល្មម មិនលឿនពេក
            "pitch": -1.0 if gender == "MALE" else 1.0 # បន្ថយ Pitch ឱ្យសំឡេងប្រុសធ្ងន់ច្បាស់
        }
    }
...
def generate_synced_audio_km(srt_raw_text, api_key):
    ...
    for idx, sub in enumerate(subs):
        ...
        status_text.markdown(f"🎙️ **កំពុងបង្កើតសំឡេង ({gender}) ផ្នែកទី {idx+1}/{total_subs}...**")

        # ១. បង្កើត Silence ន្លោះពេលកាត់តាម Timecode ដើម្បឱ្យអានដកដង្ហើមតាមចង្វាក់
        ...
        # ២. បង្កើត Audio និយាយ (បែងចែកប្រុស/ស្រី)
        if cleaned_text:
            audio_path = os.path.join(temp_dir, f"speech_{idx}.mp3")
            if get_google_cloud_tts_voice(cleaned_text, gender, api_key, audio_path):
    ...
    # ៣. ផ្គុំ File ទាំងអស់ចូលគ្នាដោយប្រើ FFmpeg Concatenate
...
def clear_text_auto_callback():
    """ មុខងារលុបអត្ថបទក្នុង Text Area ដោយស្វ័យប្រវត្តិ """
...
with tab1:
    ...
    with col_polish2:
        st.button(
            "🧹 Clear Input (លុបអត្ថបទអូតូ)",
            key="tab1_clear_text_btn",
            use_container_width=True,
            on_click=clear_text_auto_callback,
        )
    ...
    if gen_clicked:
        if not active_api_key:
            st.error("❌ សូម Paste Gemini API Key នៅក្នុង Sidebar ជាមុនសិនដើម្បីដោះសោសំឡេង HD ប្រុស/ស្រី!")
        elif not srt_content.strip():
            st.warning("⚠️ សូមបញ្ចូលអត្ថបទ SRT ឬអក្សរជាមុនសិន!")
        else:
            try:
                audio_bytes = generate_synced_audio_km(srt_content, active_api_key)
