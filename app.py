...
def fetch_edge_audio_sync(text, voice):
    ...
    # បន្ថែម --rate=-4% ដើម្បសម្រួលល្បឿនសំឡេងឱ្យត្រូវតាមចង្វាក់ habla SRT (អាចផ្លាស់ប្តូរលេខ percent តាមចិត្ត)
...
def generate_synced_audio_km(srt_raw_text, voice_mode_selected):
    ...
    # ១. បន្ថែមប្រមាណ Bytes សម្រាប់បង្កើតចន្លោះស្ងាត់ (Silence Padding)
    ...
    for idx, sub in enumerate(subs):
        ...
        # ២. គណនាចន្លោះពេលស្ងាត់រវាងប្រយោគ ដើម្បីឱ្យសំឡេងមានចន្លោះស្ងាត់ដូច App គេ
        start_sec = sub.start.total_seconds()
        gap = start_sec - last_end_time

        if gap > 0.1:
            silence_bytes = b"\x00" * int(gap * BYTES_PER_SECOND)
        ...
        # ៣. កត់ត្រា Timecode បញ្ចប់នៃប្រយោគ
