...
with st.sidebar:
    ...
    trans_api = st.radio(
        "", ["Gemini Api", "Google Api"], key="trans_style"
    )
        ...
        if not srt_content.strip():
            ...
            if not processed_lines:
                ...
                for idx, (text, gender) in enumerate(processed_lines):
                    ...
                    try:
                        audio_stream = asyncio.run(
                            generate_voice_bytes(text, v_code)
                        )
                        ...
                        st.error(
                            f"កំហុសត្រង់ជួរទី {idx+1} ៖ Microsoft TTS មិនឆ្លើយតប"
                        )
                ...
                if len(final_audio_bytes) > 0:
                    ...
                    st.session_state["audio_data"] = final_audio_bytes
                    st.session_state["audio_time"] = int(time.time())
                else:
                    st.error("❌ ការបង្កើតសំឡេងបរាជ័យ ៖ គ្មានទិន្នន័យសំឡេង!")

    if "audio_data" in st.session_state:
        dynamic_key = f"player_{st.session_state['audio_time']}"
        st.audio(
        ...
        final_filename = (
            f"{file_custom_name}.mp3"
            if file_custom_name.strip()
            else "Vichhai_Dubbed_Output.mp3"
        )
...
with tab3:
    ...
    st.info(
        "មុខងារបង្កប់ Subtitle ចូលទៅក្នុង Video (Hardcode Subtitles)"
    )
