...
st.markdown(
    """
<style>
    /* បង្ខំ Background កម្មវិធីឱ្យងងឹត ១០០% លើទូរស័ព្ទ (ដោះស្រាយបញ្ហាក្រឡាស) */
...
async def generate_synced_audio(srt_raw_text, voice_mode_selected):
    """ បង្កើត Audio ដោយប្រើ Pydub AudioSegment ដើម្បីឱ្យ iOS/CapCut ស្គាល់ 100% អាចទាញចូល Timeline បាន """
    ...
    for idx, sub in enumerate(subs):
        ...
        if cleaned_text:
            ...
            audio_chunk_bytes = await get_audio_bytes(cleaned_text, v_code)
            if audio_chunk_bytes:
                segment_audio = AudioSegment.from_file(io.BytesIO(audio_chunk_bytes), format="mp3")
    ...
    last_sub_end_ms = int(subs[-1].end.total_seconds() * 1000)
    if len(final_audio) < last_sub_end_ms:
        final_audio += AudioSegment.silent(duration=(last_sub_end_ms - len(final_audio)))

    out_buffer = io.BytesIO()
    final_audio.export(out_buffer, format="mp3", bitrate="128k")
