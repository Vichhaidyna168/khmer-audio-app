...
def make_ffmpeg_silence(duration_sec):
...
async def generate_edge_audio_bytes(text, voice):
    try:
...
def get_working_gemini_model_name(selected_name):
    """ បំប្លែងឈ្មោះ Model Custom ទៅកាន់ Model ដែលដំណើរការស្អក និងផ្លូវការលើ Google API """
    mapping = {
        ...
        "gemini-3.5-pro-preview": "gemini-1.5-pro",
        "gemini-3-flash-preview": "gemini-2.0-flash",
    }
    return mapping.get(selected_name, "gemini-2.0-flash")
...
def polish_srt_with_gemini(srt_text, api_key, model_name):
    ...
    try:
        ...
        try:
            # បម្រុងទុក (Fallback) ករណី Model ចម្បងមានបញ្ហា
...
def generate_synced_audio_km(srt_raw_text, voice_mode_selected):
    ...
    try:
        ...
        raise Exception("ទម្រង់ SRT មានបញ្ហា! សូមពិនិត្យមើលលេខរៀង និង Timecode នៃ SRT ឡើងវិញ civilization។")
    ...
    for idx, sub in enumerate(subs):
        ...
        if start_sec > current_time_sec:
            ...
            silence_pcm = make_ffmpeg_silence(gap_sec)
            if silence_pcm:
                combined_bytes.extend(silence_pcm)
            current_time_sec = start_sec

        if cleaned_text:
            audio_chunk = loop.run_until_complete(generate_edge_audio_bytes(cleaned_text, chosen_voice))
            if not audio_chunk:
                try:
                    tts = gTTS(text=cleaned_text, lang="km", slow=False)
                    fp = io.BytesIO()
                    tts.write_to_fp(fp)
                    audio_chunk = fp.getvalue()
                except Exception:
                    pass
            ...
            if audio_chunk:
                ...
                silence_pcm = make_ffmpeg_silence(duration)
                if silence_pcm:
                    combined_bytes.extend(silence_pcm)
