...
# MP3 Silent Frame ស្ដង់ដារ (MPEG-2 Layer III, 24kHz, 32kbps Mono - 192 Bytes/Frame = 0.024s)
SILENT_MP3_FRAME = (
    bytes.fromhex("fff350c4000000034800000000000578") + b"\x00" * 176
)
...
def make_mp3_silence_bytes(duration_sec):
    if duration_sec <= 0.05:
        return b""
    # 1 frame = 0.024 វិនាទី
    num_frames = int(duration_sec / 0.024)
    return SILENT_MP3_FRAME * num_frames
...
async def generate_synced_audio(srt_raw_text, voice_mode_selected):
    ...
    for idx, sub in enumerate(subs):
        ...
        # បន្ថែមចន្លោះស្ងាត់ (Silence Gap)
        ...
        if not srt_content.strip():
            ...
            try:
                ...
                if audio_bytes and len(audio_bytes) > 1000:
                    ...
                    st.error(
                        "❌ បរាជ័យ៖ មិនអាចទាញយកសំឡេងពី Edge-TTS បានទេ! សូមព្យាយាមចុច Generate ម្តងទៀត"
                    )
